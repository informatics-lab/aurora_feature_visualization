...
def jitter_3d(max_shift):
    ...
    def inner(
        ...
        """
        Randomly jitter a surface‐variable tensor in latitude/longitude.

        Args:
            data (torch.Tensor): input tensor of shape (1, T, 1, H, W).
            max_shift (int): maximum absolute pixel shift in both axes.
            mode (str): interpolation mode, 'bilinear' or 'nearest'.
            padding_mode (str): padding for outside values: 'zeros'|'border'|'reflection'.
            align_corners (bool): align_corners for grid generation.

        Returns:
            torch.Tensor: jittered tensor, same shape as input.
        """
        assert data.ndim == 5 and data.shape[0] == 1 and data.shape[2] == 1, (
            "Expected shape (1, T, 1, H, W)"
        )
        B, T, C, H, W = data.shape
        # Flatten batch & time → batch of T images
        imgs = data.view(B * T, C, H, W)

        # Generate random integer shifts in [-max_shift, +max_shift]
        # one (dx, dy) per image
        ...
        # Build affine matrices M of shape (B*T, 2, 3)
        #    [1 0 tx]
        #    [0 1 ty]
        ...
        # Restore original five‐dim shape
