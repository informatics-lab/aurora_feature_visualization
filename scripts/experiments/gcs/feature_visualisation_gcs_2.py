...
params, image_f = image.image(lat, lon, time, lvl_type="surf", device=device)


def build_batch(image_f):
    batch = Batch(
        surf_vars={
            k: image_f[0]()[:, :, i].squeeze(axis=2)
            for i, k in enumerate(("2t", "10u", "10v", "msl"))
        },
        static_vars={k: torch.randn(721, 1440) for k in ("lsm", "z", "slt")},
        atmos_vars={
            k: torch.randn(1, 2, 4, 721, 1440) for k in ("z", "u", "v", "t", "q")
        },
        metadata=Metadata(
